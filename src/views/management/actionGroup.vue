  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select clearable style="width: 90px" class="filter-item" v-model="listQuery.GroupType" :placeholder="$t('table.grouptype')">
        <el-option v-for="item in groupOptions" :key="item.key" :label="item.label" :value="item.key">
        </el-option>
      </el-select>
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column min-width="110px" align="center" :label="$t('table.groupsort')">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="100px" style='width: 400px; margin-left:50px;'>
        <el-form-item :label="$t('table.groupname')" prop="GroupName">
          <el-input v-model="temp.GroupName" auto-complete="off"></el-input>
        </el-form-item>
        <el-form-item :label="$t('table.grouptype')">
          <el-select class="filter-item" v-model="temp.GroupType" placeholder="请选择权限组">
            <el-option v-for="item in groupOptions" :key="item.key" :label="item.label" :value="item.key">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item :label="$t('table.groupsort')" :rules="[
          { required: true, message: '年龄不能为空'},
          { type: 'number', message: '年龄必须为数字值'}
        ]">
          <el-input v-model.number="temp.Sort" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            updateActionGroups(tempData).then(() => {
